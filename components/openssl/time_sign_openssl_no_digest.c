...
int main(int argc, char *argv[]) {
    ...
    if ((evp_pkey = PEM_read_PrivateKey(fp, NULL, NULL, NULL)) == NULL){
        fprintf(stderr, "Can't read priv key.\n");
        goto err;
    }

    if ((pkey = EVP_PKEY_get1_EC_KEY(evp_pkey)) == NULL){
        fprintf(stderr, "Can't convert priv key to EC_key.\n");
        goto err;
    }
    ...
    while ((r_ret = read(in_fd, data, data_size)) > 0) {
        ...
        r_ret = ECDSA_sign_setup(pkey, bnctx, &kinv, &rp);
        if(r_ret == 0){
        ...
        r_ret = ECDSA_sign_ex(NULL, data, data_size, sig, &sig_len, kinv, rp, pkey);
        if(r_ret == 0){
            fprintf(stderr, "ECDSA_sign_ex\n");
            goto err;
        }
